...
export abstract class BaseKafkaConsumer {
  ...
  public async processMessage(context: EachMessagePayload): Promise<void> {
    ...
    try {
      ...
      kafkaObservabilityService.recordConsumerSuccess(this.consumerGroup, topic, processingTimeMs);
      ...
      kafkaObservabilityService.recordConsumerFailure(this.consumerGroup, topic, error.message || 'Processing failure');

      logStructured({
        level: 'error',
        message: `Error processing Kafka event in '${this.consumerGroup}'`,
        consumerGroup: this.consumerGroup,
        eventId: envelope.eventId,
        eventType: envelope.eventType,
        topic,
